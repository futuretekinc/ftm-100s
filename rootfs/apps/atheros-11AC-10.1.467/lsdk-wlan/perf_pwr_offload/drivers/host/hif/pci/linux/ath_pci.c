...
#if 0
    tasklet_hi_schedule(&sc->intr_tq);
#else
...
ath_tasklet(unsigned long data)
{
    ...
    if (LEGACY_INTERRUPTS(sc)) {
#ifdef CONFIG_CS75XX_WFO_AR988X
        u32 intr_data;
        intr_data = PCIE_INTR_FIRMWARE_MASK | PCIE_INTR_CE_MASK_ALL;
        if (CS_AR988X_WFO_enabled(sc)) {
#ifdef CONFIG_CS75XX_WFO_AR988X_RX
            /*
             * when enable WFO rx, we need to disable RX CE PCIe interrupt
             */
            if (CS_A988X_CE_HTT_RX_ID < CE_COUNT) {
                u32 data = (0x00000800 << CS_A988X_CE_HTT_RX_ID);
                intr_data &= ~data;
             }
#endif /* CONFIG_CS75XX_WFO_AR988X_RX */
#ifdef CONFIG_CS75XX_WFO_AR988X_TX
            if (CS_A988X_CE_HTT_RX_ID < CE_COUNT) {
                u32 data = (0x00000800 << CS_A988X_CE_HTT_TX_ID);
                intr_data &= ~data;
            }
        ...
        A_PCI_WRITE32(sc->mem+(SOC_CORE_BASE_ADDRESS | PCIE_INTR_ENABLE_ADDRESS),
            intr_data);
#else /* CONFIG_CS75XX_WFO_AR988X */    	
        /* Enable Legacy PCI line interrupts */
        A_PCI_WRITE32(sc->mem+(SOC_CORE_BASE_ADDRESS | PCIE_INTR_ENABLE_ADDRESS), 
		    PCIE_INTR_FIRMWARE_MASK | PCIE_INTR_CE_MASK_ALL); 
#endif	/* CONFIG_CS75XX_WFO_AR988X */	    
        /* IMPORTANT: this extra read transaction is required to flush the posted write buffer */
        tmp = A_PCI_READ32(sc->mem+(SOC_CORE_BASE_ADDRESS | PCIE_INTR_ENABLE_ADDRESS));
    }
}
...
#ifdef CONFIG_CS75XX
 	/* do this regardless of FORCE_LEGACY_PCI_INTERRUPTS */
    if (!msi_enable) {
        num_msi_desired = 0;
    }
#endif /* CONFIG_CS75XX */
