...
#ifdef CONFIG_CS75XX_WFO_AR988X_RX
        { /* CE1 */ CE_ATTR_FLAGS, 0, 0, CS_A988X_CE_HTT_RX_MAX_SIZE, CS_A988X_CE_HTT_RX_DST_NENTRIES, NULL, },/* target->host HTT + HTC control */
#else                                                           
...
#ifdef CONFIG_CS75XX_WFO_AR988X_TX
    if ((hif_state->sc->wfo_enabled) && (pipe == CS_A988X_CE_HTT_TX_ID)) {
        CS_AR988X_PNI_TX_CE_SKB(hif_state->sc, nbytes, nbuf);
        return A_OK;
    }
#endif
    /*
...
#ifdef CONFIG_CS75XX_WFO_AR988X_RX
        if (pipe_num != CS_A988X_CE_HTT_RX_ID) {
#endif
        pipe_info = &hif_state->pipe_info[pipe_num];
...
#ifdef CONFIG_CS75XX_WFO_AR988X
    CS_AR988X_WFO_PNI_init(hif_state->sc);
#endif
...
HIF_PCIDeviceProbed(hif_handle_t hif_hdl)
{
    ...
    printk("%s CONFIG_CS75XX_WFO_AR988X targid@sc=%x hif_state->targid=%x phy_addr start=%x end=%x PCI_ADDR=%x sc->mem=%x\n", __func__,
        TARGID(sc), hif_state->targid, dev->mem_start, dev->mem_end, TARGID_TO_PCI_ADDR(hif_state->targid) , sc->mem);

    sc->wfo_enabled = 0;
#endif
#ifdef CONFIG_CS75XX_WFO_AR988X_RX
    printk("%s CONFIG_CS75XX_WFO_AR988X_RX \n", __func__);
#endif

#ifdef CONFIG_CS75XX_WFO_AR988X_TX
    printk("%s CONFIG_CS75XX_WFO_AR988X_TX \n", __func__);
#endif
...
/*
 * forward CE messages which from HIF layer to HTC layer
 */
int HIF_WFO_RX_completion(void * ce_context, void * skb)
{
    ...
    while (atomic_read(&hif_state->fw_event_pending) > 0) {
        atomic_set(&hif_state->fw_event_pending, 0);
        printk("%s msg_callbacks->fwEventHandler=%p\n", __func__, msg_callbacks->fwEventHandler);
        msg_callbacks->fwEventHandler(msg_callbacks->Context);

    }
	//printk("%s msg_callbacks->rxCompletionHandler=%p\n", __func__, msg_callbacks->rxCompletionHandler);


    /*
     * refer to  == >hif_completion_thread () which will direct sent to HTCRxCompletionHandler()
     */
    msg_callbacks->rxCompletionHandler(msg_callbacks->Context, skb, pipe_info->pipe_num);
    return 0;
}
...
int HIF_WFO_TX_completion(void * ce_context, void * skb)
{
	...
	//printk("%s msg_callbacks->txCompletionHandler=%p\n", __func__, msg_callbacks->rxCompletionHandler);

    /*
     * refer to  == >hif_completion_thread () which will direct sent to HTCTxCompletionHandler()
     */
    msg_callbacks->txCompletionHandler(msg_callbacks->Context, skb, 1 /*transfer_id*/);
    return 0;
}
