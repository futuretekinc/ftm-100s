...
CE_per_engine_servicereap(struct ath_hif_pci_softc *sc, unsigned int CE_id)
{
	...
	if (CE_id == CS_A988X_CE_HTT_TX_ID) {
		if (CS_AR988X_WFO_enabled(sc)) {
			return;
		}
	}
#endif
...
CE_per_engine_service(struct ath_hif_pci_softc *sc, unsigned int CE_id)
{
    ...
    if (CE_id == CS_A988X_CE_HTT_RX_ID) {
        if (CS_AR988X_WFO_enabled(sc)) {
            return;
        }
        printk("write_idx=%d read_idx=%d\n", CE_DEST_RING_WRITE_IDX_GET(targid, ctrl_addr),
            CE_DEST_RING_READ_IDX_GET(targid, ctrl_addr));
    }
#endif
#ifdef CONFIG_CS75XX_WFO_AR988X_TX
    if (CE_id == CS_A988X_CE_HTT_TX_ID) {
        ...
        struct CE_attr *attr)
{
    ...
    if (nentries) {
		...
		A_MALLOC(nentries * sizeof(struct CE_src_desc) + CE_DESC_RING_ALIGN);
		base_addr = pci_map_single(NULL, 
			src_ring->base_addr_owner_space_unaligned ,	nentries * sizeof(struct CE_src_desc) + CE_DESC_RING_ALIGN, ADF_OS_DMA_TO_DEVICE);
...
#ifdef CONFIG_CS75XX_WOT
		A_MALLOC(nentries * sizeof(struct CE_dest_desc) + CE_DESC_RING_ALIGN);
		base_addr = pci_map_single(NULL, 
			dest_ring->base_addr_owner_space_unaligned ,	nentries * sizeof(struct CE_dest_desc) + CE_DESC_RING_ALIGN, ADF_OS_DMA_TO_DEVICE);
		base_addr |= 0x40000000;
#else
                OS_MALLOC_CONSISTENT(scn->sc_osdev,
...
#ifdef CONFIG_CS75XX_WOT
		A_FREE(CE_state->src_ring->base_addr_owner_space);
#else
...
#endif /* QCA_OL_11AC_FAST_PATH */
#ifdef CONFIG_CS75XX_WOT
		A_FREE(CE_state->dest_ring->base_addr_owner_space);
#else
		OS_FREE_CONSISTENT(scn->sc_osdev,
