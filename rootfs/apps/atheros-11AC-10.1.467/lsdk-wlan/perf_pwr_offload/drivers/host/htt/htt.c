...
#ifdef CONFIG_CS75XX_WFO_AR988X
		printk("%s ol_cfg_tx_download_size(pdev->ctrl_pdev)=%d \n", __func__, ol_cfg_tx_download_size(pdev->ctrl_pdev));
		printk("%s sizeof(struct htt_host_tx_desc_t)=%d \n", __func__, sizeof(struct htt_host_tx_desc_t));

		if (pdev->download_len != CS_A988X_PDEV_DOWNLOAD_LEN)
			printk("WARNING?? %s:%d WFO CS_A988X_PDEV_DOWNLOAD_LEN=%d != download_len=%d	\n", __func__, __LINE__, CS_A988X_PDEV_DOWNLOAD_LEN, pdev->download_len);
#endif
...
#ifdef CONFIG_CS75XX_WFO_AR988X
	if (pdev->htc_endpoint != CS_A988X_PDEV_TX_HTC_ENDPOINT)
		printk("WARNING?? %s:%d WFO CS_A988X_PDEV_TX_HTC_ENDPOINT=%d != pdev->htc_endpoint=%d	\n",
			__func__, __LINE__, CS_A988X_PDEV_TX_HTC_ENDPOINT, pdev->htc_endpoint);
#endif
    return 0; /* success */
}
