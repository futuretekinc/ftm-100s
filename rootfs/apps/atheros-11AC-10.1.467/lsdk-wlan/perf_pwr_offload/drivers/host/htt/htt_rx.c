...
htt_rx_ring_size(struct htt_pdev_t *pdev)
{
    ...
    return CS_A988X_HTT_RX_RING_SIZE;
#endif
    /*
...
#ifdef CONFIG_CS75XX_WFO_AR988X_RX
void
CS75xx_WFO_htt_rx_ring_fill_n(struct htt_pdev_t *pdev, int num)
{
    int idx;
    a_status_t status;
    ...
    printk("%s: idx %d\n", __func__, idx);
    printk("\t rx based addr=0x%08x fill_num=%d\n", CS_A988X_HTT_RX_BUFFER_PADDR, num);
    while (num > 0) {
        u_int32_t paddr;
        char *rx_netbuf;

        //rx_netbuf = adf_nbuf_alloc(pdev->osdev, HTT_RX_BUF_SIZE, 0, 4, FALSE);
        rx_netbuf = CS_A988X_HTT_RX_BUFFER_PADDR + HTT_RX_BUF_SIZE * (num-1);
        /* Clear rx_desc attention word before posting to Rx ring */
        //rx_desc = (struct htt_host_rx_desc_base *)adf_nbuf_data(rx_netbuf);
        rx_desc = rx_netbuf;
        //*(u_int32_t *)&rx_desc->attention = 0;

        //paddr = adf_nbuf_get_frag_paddr_lo(rx_netbuf, 0);
        //pdev->rx_ring.buf.netbufs_ring[idx] = rx_netbuf;
        pdev->rx_ring.buf.paddrs_ring[idx] = rx_netbuf;
...
#ifndef CONFIG_CS75XX_WFO_AR988X_RX
...
#ifdef CONFIG_CS75XX_WFO_AR988X_RX
        pdev->rx_ring.base_paddr = CS_A988X_HTT_RX_DSC_PADDR;
        pdev->rx_ring.buf.paddrs_ring = CS_A988X_HTT_RX_DSC_PADDR;
        pdev->rx_ring.alloc_idx.paddr = CS_A988X_HTT_RX_PADDR;
        pdev->rx_ring.alloc_idx.vaddr = CS_A988X_HTT_RX_PADDR;
#else
...
#ifdef CONFIG_CS75XX_WFO_AR988X_RX
        CS75xx_WFO_htt_rx_ring_fill_n(pdev, pdev->rx_ring.fill_level);
#else            
