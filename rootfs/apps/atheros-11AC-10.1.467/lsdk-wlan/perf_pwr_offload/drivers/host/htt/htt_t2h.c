...
#if 0
            printk("%s pdev->txrx_pdev=%p (msg_word + 1)=%p num_msdus=%d\n",
                __func__, pdev->txrx_pdev, (msg_word + 1), num_msdus);
            int i;
            for (i = 0; i < num_msdus; i++) {
                u_int16_t tx_desc_id;

                //tx_desc_id = htt_tx_compl_desc_id((void *) (msg_word + 1), i);
                tx_desc_id =  *(((u_int16_t *) msg_word + 1) + i);
                printk("\t tx_desc_id[%d]=%d \n", i, tx_desc_id);
            }
#endif /* 0 */
            adf_nbuf_push_head(htt_t2h_msg, HTC_HEADER_LEN + HTC_HDR_ALIGNMENT_PADDING);

            //need to send this packet to TX PE for release
            temp_CS_AR988X_PNI_TX_COMPLETE_DONE(htt_t2h_msg);
            return;
#endif /* ! CONFIG_CS75XX_WFO_AR988X_RX */

#else /* CONFIG_CS75XX_WFO_AR988X_TX */             
            ol_tx_completion_handler(
                pdev->txrx_pdev, num_msdus, status, msg_word + 1);
#endif /* CONFIG_CS75XX_WFO_AR988X_TX */              
