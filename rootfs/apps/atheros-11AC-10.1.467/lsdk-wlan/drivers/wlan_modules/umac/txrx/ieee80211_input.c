                       ...
                       u_int32_t hdrspace, int is_mcast, u_int8_t subtype) 
{
    ...
    if (wbuf != NULL) {
        ...
        /*
         * setup cs_cb for wfo hw acceleration
         */
        //printk("%s vap->iv_ic=%p vap=%p \n", __func__, vap->iv_ic, vap);
        struct ath_softc_net80211 *scn = ATH_SOFTC_NET80211(vap->iv_ic);

        if (CS_AR9580_WFO_enabled(scn)) {
            cs_hw_accel_wfo_wifi_rx(CS_WFO_CHIP_AR9580, scn->pid /*CS_WFO_IPC_PE1_CPU_ID*/ , wbuf);
{
    	    /* Bug#39840: wfo mode 9; dbdc; can't ping 11n from g2-eth1 pc */
            cs_kernel_accel_cb_t *cs_cb = CS_KERNEL_SKB_CB(wbuf);;
    		if (cs_cb != NULL) {
