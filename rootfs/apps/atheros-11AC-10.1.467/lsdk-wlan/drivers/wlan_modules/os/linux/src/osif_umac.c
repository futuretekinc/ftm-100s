...
#ifdef CONFIG_CS75XX_WFO_ARXXXX
#include <cs_core_logic.h>
#include <cs_wfo_csme.h>
cs_wfo_csme_sta_list_s csme_send_list;
#endif /* CONFIG_CS75XX_WFO_ARXXXX */
...
#ifdef CONFIG_CS75XX_WFO_ARXXXX
    memset(&csme_send_list, 0, sizeof(cs_wfo_csme_sta_list_s));
#endif /* CONFIG_CS75XX_WFO_ARXXXX */
    /*
...
int osif_ol_vap_hardstart_fast(struct sk_buff *skb, struct net_device *dev);
#endif

void wot_tx_tasklet(ulong data)
{
	struct net_device *dev = (struct net_device *)data;
	osif_dev  *osifp = ath_netdev_priv(dev);
	struct sk_buff *skb;
	int count=0, need_reschedule=0;

	while ((skb = skb_dequeue(&osifp->pending_tx)) != NULL){
		//do_osif_vap_hardstart(skb, dev);
#if QCA_OL_11AC_FAST_PATH
		if(osifp->osif_is_mode_offload)
			osif_ol_vap_hardstart_fast(skb, dev);
		else
#endif
			osif_vap_hardstart_generic(skb, dev);
//#if 0
		if(osifp->osif_is_mode_offload && (count++ >= 256)) {
//			need_reschedule = 0;
			break;
		}
#if 0
		else if((osifp->osif_is_mode_offload==0) && (count++>= 128)) {
			need_reschedule = 0;
			break;
		}
#endif
	}
	/* ...clear queue dispatching flag */
	clear_bit(1, &osifp->qdispatch);

//	if(need_reschedule == 1)
//		tasklet_schedule(&osifp->tx_tasklet);

}

void wot_rx_tasklet(ulong data)
{
	struct net_device *dev = (struct net_device *)data;
	osif_dev  *osifp = ath_netdev_priv(dev);
	struct sk_buff *skb;
	...
	set_bit(0, &osifp->qdispatch);
	while ((skb = skb_dequeue(&osifp->pending_rx)) != NULL) {
		/* ...packet is going up the stack; complete buffer mapping */
		//if (!br_fdb_test_forward(skb->dev, skb->mac_header)) {
			/* ...packet is going up the stack; complete buffer mapping */
			//PKTCTFMAP(wl->osh, skb);
		//}

		/* Jason, dont't push to backlog queue */
#ifdef CONFIG_CS75XX_HW_ACCEL_WIRELESS
//		if (cs_hw_accel_wireless_rx(skb) != 0)
#endif
		netif_receive_skb(skb);
		//netif_rx(skb);

		if(osifp->osif_is_mode_offload & (++count > 512)){ /* Yield CPU to prevent starvation */
				need_reschedule = 1;
				break;
		}
	}
	...
	if(need_reschedule == 1) {
		tasklet_schedule(&osifp->rx_tasklet);
	} else {
		/* ...clear queue dispatching flag */
		clear_bit(0, &osifp->qdispatch);
	}
}

/* ...schedule tasklet execution - trampoline for processing transfer to other core */
static void wot_sched_tasklet(void *data)
{
...
/* ...schedule tx/rx queue processing on proper CPU */
int wot_queue_dispatch(osif_dev *osif, int q)
{
	int target_cpu = smp_processor_id() ^ 1;

	if (q == 0) {
		/* ...trigger receive queue processing on target CPU (netif_rx) */
		if (!test_and_set_bit(0, &osif->qdispatch) ) {
			return smp_call_function_single(target_cpu, wot_sched_tasklet, &osif->rx_tasklet, 1);
		}
	} else {
		/* ...traigger transmission queue processing */
		if (!test_and_set_bit(1, &osif->qdispatch)) {
			return smp_call_function_single(target_cpu, wot_sched_tasklet, &osif->tx_tasklet, 1);
		}
	}
...
#ifdef CONFIG_CS75XX_WOT
    osifp->qdispatch = 0;
    printk(")))))))))))) Init tasklet for WOT ((((((((((((\n");
    tasklet_init(&osifp->tx_tasklet, wot_tx_tasklet, (unsigned long)dev);
    tasklet_init(&osifp->rx_tasklet, wot_rx_tasklet, (unsigned long)dev);
    /* ...pending packets queue */
    skb_queue_head_init(&osifp->pending_rx);
    skb_queue_head_init(&osifp->pending_tx);
#endif
...
#ifdef CONFIG_CS75XX_WFO_ARXXXX
static void process_cs_csme_skb_copy(struct sk_buff *skb, adf_os_device_t osdev, unsigned char ad_id)
{
    cs_wfo_csme_status_e ret;
    struct sk_buff *skb_new, *skb_old;
    int i;
    ...
    if (cs_cb == NULL) {
         ...
         */
        adf_nbuf_map_single(
            osdev, (adf_nbuf_t) skb, ADF_OS_DMA_TO_DEVICE);
        /* terminate the (single-element) list of tx frames */
        skb->next = NULL;
        return;
    }

//printk("### %s::%d, skb->data %pM, module_mask 0x%x, egress_port_id 0x%x\n"
//        , __func__, __LINE__, skb->data
//        , cs_cb->common.module_mask, cs_cb->common.egress_port_id);

    ret = cs_wfo_csme_group_get_member(skb->data, ad_id, &csme_send_list);
    if (ret == CS_WFO_CSME_STATUS_SUCCESS){
//        printk(KERN_NOTICE "%s:: da %pM, ret SUCCESS, \n", __func__, skb->data);
//        printk(KERN_NOTICE "%s:: Total count %d, hw_count %d, sw_count %d\n",
//              __func__, csme_send_list.total_count, csme_send_list.hw_count, csme_send_list.sw_count);

        if (csme_send_list.total_count) {
//            printk(KERN_NOTICE "%s(%d):: (1) mac_da %pM, egress_port_id %x\n",
//                    __func__, __LINE__, csme_send_list.sta_list[0].mac_da,
//                    csme_send_list.sta_list[0].egress_port_id);
            memcpy(skb->data, csme_send_list.sta_list[0].mac_da, 6);
            cs_cb->common.egress_port_id = csme_send_list.sta_list[0].egress_port_id;
            /*
             * DMA mapping is done within the OS shim prior to sending
             * the frame to the driver.
             */
            ...
            skb_old = skb;
            for (i=1; i<csme_send_list.total_count; i++) {
                skb_new = skb_copy(skb, GFP_ATOMIC);
                if (skb_new != NULL) {
//                    printk(KERN_NOTICE "%s(%d):: (%d) mac_da %pM, egress_port_id %x\n",
//                            __func__, __LINE__, i+1, csme_send_list.sta_list[i].mac_da,
//                            csme_send_list.sta_list[i].egress_port_id);
                    memcpy(skb_new->data, csme_send_list.sta_list[i].mac_da, 6);
                    cs_cb->common.egress_port_id = csme_send_list.sta_list[i].egress_port_id;
                    /*
                     * DMA mapping is done within the OS shim prior to sending
                     * the frame to the driver.
                     */
                    adf_nbuf_map_single(
                        osdev, (adf_nbuf_t) skb_new, ADF_OS_DMA_TO_DEVICE);
                    /* terminate the (single-element) list of tx frames */
                    skb_new->next = NULL;
                }/* if (skb_new != NULL) */

                skb_old->next = skb_new;
                skb_old = skb_new;
            }/* for (i=0; i<csme_send_list.hw_count; i++) */
        }/* if (csme_send_list.total_count) */
    } else {
//         printk(KERN_NOTICE "%s:: da %pM, ret FAILED, \n", __func__, skb->data);
        /*
         * DMA mapping is done within the OS shim prior to sending
         * the frame to the driver.
         */
...
#endif
    {
        if (osdev->osif_is_mode_offload) {
            ...
            if (is_multicast_ether_addr(skb->data) && !is_broadcast_ether_addr(skb->data)) {
                // CSME support
                process_cs_csme_skb_copy(skb, osdev, CS_WFO_CSME_WIFI_0);
            } else {
#endif /* CONFIG_CS75XX_WFO_ARXXXX */
            /*
             * DMA mapping is done within the OS shim prior to sending
            ...
            if (is_multicast_ether_addr(skb->data) && !is_broadcast_ether_addr(skb->data)) {
                // 11N CSME feature
                process_cs_csme_skb_copy(skb, osdev, CS_WFO_CSME_WIFI_1);
            }
#endif /* CONFIG_CS75XX_WFO_ARXXXX */
...
#ifdef CONFIG_CS75XX_WOT
    osif_dev  *osdev = ath_netdev_priv(dev);
    /* free skb if too many pending frame */
    if(skb_queue_len(&osdev->pending_tx) >= 16384){
		dev_kfree_skb_any(skb);
		return 0;
    }
	
    skb_queue_tail(&osdev->pending_tx, skb);
    if(wot_queue_dispatch(osdev, 1))
            printk("failed to enqueue skb on CPU#0\n");
#else
    do_osif_vap_hardstart(skb,dev);
#endif

#ifdef CONFIG_CS75XX
    /* debug_Aaron on 2013/04/26 return 0 for fast bridge */
    return 0;
#endif /* CONFIG_CS75XX */
}
...
#else
    send_err = ol_tx_ll_fast(osdev->iv_txrx_handle, &skb, num_msdus);
#endif
    /*
...
#ifdef CONFIG_CS75XX_WOT
	tasklet_kill(&osifp->rx_tasklet);
	tasklet_kill(&osifp->tx_tasklet);
#endif
