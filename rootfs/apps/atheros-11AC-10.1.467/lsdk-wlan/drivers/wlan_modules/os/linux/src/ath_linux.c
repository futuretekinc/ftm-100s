...
#ifdef CONFIG_CS75XX
    struct pci_dev *pdev = (struct pci_dev *)scn->sc_osdev->bdev;
#endif /* CONFIG_CS75XX */
...
//#if 0
    tasklet_hi_schedule(&scn->sc_osdev->intr_tq);
#else
   ...
   OS_MESGQ_INIT(osdev, &osdev->async_q, sizeof(os_async_q_mesg),
...
#else /* ! CONFIG_CS75XX */
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,20)
    if (request_irq(dev->irq, ath_isr, 0, dev->name, dev)) {
#else
    ...
    if (pdev->msi_enabled) {
       printk(KERN_INFO "%s : MSI WA timer is enabled\n", __func__);
        ...
        OS_INIT_TIMER(scn->sc_osdev, &scn->msi_poll_timer, ath_msi_poll_timer, scn);
