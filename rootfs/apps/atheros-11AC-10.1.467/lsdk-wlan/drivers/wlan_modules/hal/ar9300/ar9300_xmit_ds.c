...
ar9300_proc_tx_desc(struct ath_hal *ah, void *txstatus)
{
    ...
    */
    ...
    ts->tid = MS(ads->status8, AR_tx_tid);
...
#ifdef CONFIG_CS75XX_WFO_AR9580
       if ((ads->status1 & AR_tx_status_rsvd) == 0xff) {
               printk("%s get fake TX status from PE \n", __func__);
       ...
       if ((ads->status1 & AR_tx_status_rsvd) != 0) {
                       ts->ts_wfo_flags |= HAL_TX_WFO_RATE_REPORT;
                       ts->ni_keyidx = (ads->status1 & AR_tx_status_rsvd) >> 8;
            ts->ba_low = ads->status5;
            ts->ba_high = 0xefef;
       }
#endif
