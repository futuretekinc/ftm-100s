...
{
    u_int32_t paddr_lo, len;

#ifdef CONFIG_CS75XX
    //debug_Aaron on 2013/04/22 for BUG#39012, pass wrong device to pci_unmap_single() will cause system panic
    struct pci_dev *pdev = to_pci_dev(osdev->dev);
#endif /* CONFIG_CS75XX */
...
#ifdef CONFIG_CS75XX
    if (buf->head_pa) {
    	NBUF_MAPPED_PADDR_LO(buf) = paddr_lo = buf->head_pa + (buf->data - buf->head);
		return A_STATUS_OK;
    } else {
#ifdef CONFIG_CS75XX_WOT
    	if(!buf->dirty_buffer && (buf->len>1200))
			len = 3 * SMP_CACHE_BYTES;
		else
			len = buf->end- buf->data;

		NBUF_MAPPED_PADDR_LO(buf) = paddr_lo =
			pci_map_single(pdev, buf->data,	len, dir);
#else
    /* assume that the OS only provides a single fragment */
    NBUF_MAPPED_PADDR_LO(buf) = paddr_lo =
	pci_map_single(pdev, buf->data,
    ...
    adf_os_device_t osdev, adf_nbuf_t buf, adf_os_dma_dir_t dir)
{
	...
	if (buf->head_pa == NULL)
		pci_unmap_single(pdev, NBUF_MAPPED_PADDR_LO(buf),
                     buf->end - buf->data, dir);
#endif
#else /* CONFIG_CS75XX */
#if !defined(A_SIMOS_DEVHOST)
