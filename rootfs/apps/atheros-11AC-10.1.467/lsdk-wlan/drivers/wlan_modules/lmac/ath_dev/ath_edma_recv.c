...
ath_rx_edma_init(ath_dev_t dev, int nbufs)
{
    ...
    do {
        ...
        printk("%s struct ath_softc *sc=%p ath_dev_t dev=%p sc->sc_ieee=%p\n", __func__, sc, dev, sc->sc_ieee);
       ...
       printk("%s CONFIG_CS75XX_WFO_AR9580_TX \n", __func__);
        ...
        for (i = 0; i < nbufs; i++, bf++, rxs++) {
            ...
            bf->bf_buf_addr[0] = bf->bf_dmacontext = virt_to_phys(wbuf->data) | 0x40000000;
#else
...
ath_edma_startrecv(struct ath_softc *sc)
{
...
#ifdef CONFIG_CS75XX_WFO_AR9580_RX
    if (CS_AR9580_WFO_enabled(sc->sc_ieee) == 0) {
#endif
    ...
    DPRINTF(sc, ATH_DEBUG_RX_PROC, "%s[%d]: RxFifoHeadLP %d RxFifoTailLP %d\n", __func__, __LINE__,
        ...
        CS_AR9580_WFO_send_reset_rx(sc->sc_ieee);
    }
#endif
...
#ifdef CONFIG_CS75XX_WFO_AR9580_RX
    /*
     * If WFO RX enable, the rxqueue will be filled by PNIC
     */
    if (CS_AR9580_WFO_enabled(sc->sc_ieee) == 0)
#endif
    // Retrieve completed receive buffers from HW. 
...
#ifdef CONFIG_CS75XX_WFO_AR9580
EXPORT_SYMBOL(ath_rx_handler);
EXPORT_SYMBOL(ath_rx_process_uapsd);
#endif
