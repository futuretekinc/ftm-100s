    ...
    const char *name, int ndesc)
{
    ...
    if (CS_AR9580_WFO_enabled(sc->sc_ieee)) {
        /*
         * need to set PE's txs ring phy address
         * or don't update this
         */
        printk("%s WFO setup tx status ring CS_A9580_A9_TXS_VADDR=%p CS_A9580_WIFI_TXS_PADDR=%p\n",
            __func__, CS_A9580_A9_TXS_VADDR, CS_A9580_WIFI_TXS_PADDR);
        dd->dd_desc = CS_A9580_A9_TXS_VADDR;
        dd->dd_desc_paddr = CS_A9580_WIFI_TXS_PADDR;
    } else
#endif
