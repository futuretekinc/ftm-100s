    ...
    )
{
    ...
    if (statuscode == IEEE80211_STATUS_SUCCESS) {
		...
		/*
         * for 11n WFO, we use the default baw_size 64
         * need to monitor if any station is setup for baw_size is not 64
         */
        printk("%s ADDBA update baw_size (%d) for tid (%d) \n", __func__, tid->baw_size, tidno);
#endif
...
#define MS(_v, _f)  (((_v) & _f) >> _f##_S)
        ...
        if (is_ap && !mimoburst && is_ms_rate) {
            ...
            if (an->an_smmode == ATH_SM_PWRSAV_DYNAMIC) {
               ...
               if (nframes)
               {
                       struct ieee80211_frame *wh = NULL, *wh_prev = NULL;

                       if (bf_prev)
                               wh_prev = (struct ieee80211_frame *)wbuf_header(bf_prev->bf_mpdu);
                       else
                               printk("%s:: should not happened!\n", __func__);
                       wh = (struct ieee80211_frame *)wbuf_header(bf->bf_mpdu);

                       if ((((wh->i_fc[0] & IEEE80211_FC0_TYPE_MASK) == IEEE80211_FC0_TYPE_DATA)&&
                               ((wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK) == IEEE80211_FC0_SUBTYPE_QOS_NULL)) ||
                               (((wh_prev->i_fc[0] & IEEE80211_FC0_TYPE_MASK) == IEEE80211_FC0_TYPE_DATA)&&
                               ((wh_prev->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK) == IEEE80211_FC0_SUBTYPE_QOS_NULL))) {
