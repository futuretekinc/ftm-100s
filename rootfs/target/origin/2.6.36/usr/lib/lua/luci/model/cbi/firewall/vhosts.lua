...
$Id: vhosts.lua,v 1.1.1.1 2012/09/20 11:21:01 fiona Exp $
]]--

local ds = require "luci.dispatcher"
local ft = require "luci.tools.firewall"

m = Map("firewall", translate("Firewall - Virtual Hosts"),
	translate("Virtual hosting allows remote computers on the Internet to \
...
local vhosts = {}
local videx = 1
uci:foreach("luci", "virtual_host", function(s)
      name  = s[".name"]
      port  = uci:get("luci", name, "port")
      proto = uci:get("luci", name, "proto")
      tbl = tbl .. string.format("%d: ['%s', %d],\n", port, name, proto)
      vhosts[videx] = { name= name, port= port, proto= proto }
      videx = videx + 1
end)

s = m:section(TypedSection, "redirect", translate("Virtual Hosts"))
s.template  = "cbi/tblsection"
s.addremove = true
s.anonymous = true
s.sortable  = true
s.extedit   = ds.build_url("admin/system/firewall/vhosts/%s")
s.template_addremove = "firewall/cbi_addvhost"
s.tbl = tbl
s.vhosts = vhosts
