...
function createtree()
	...
	local baselang = conf.main.i18nbase or "base"
	require "luci.i18n".loadc(baselang)
...
function entry(path, target, title, order)
	...
	local key = string.gsub(table.concat(path,'_'),'-','_')
	local lorder = nil
	if luci.config and luci.config.menu_order then
		lorder = luci.config.menu_order[key] or nil
	end
	if lorder then
		c.order = tonumber(lorder)
	else
		c.order  = order
	end
...
local function _cbi(self, ...)
	...
	-- Allow /etc/config/luci to set autoapply for
	-- any admin gui, even admin-full
	--
	local conf = require "luci.config"
        assert(conf.main,
	       "/etc/config/luci seems to be corrupt, unable to find section 'main'")
        local ap = conf.main.autoapply and not not conf.main.autoapply or false
	...
	if not config.nofooter then
		tpl.render("cbi/footer", {
			...
			redirect   = redirect,
			state      = state,
			autoapply  = config.autoapply or ap
		})
	end
end
