...
/*BUG#39963: The code breaks the codes layer policy in kernel 2.6.36*/
...
#ifndef CONFIG_CS75XX_NI_EXPERIMENTAL_SW_CACHE_MANAGEMENT
			dma_sync_single_for_cpu((struct device *)dev, rx_dma_addr,
...
//#endif
	 ...
	 */
		...
		if (packet_type == CS_APP_PKT_TUNNEL) {
			/*
			 * FIXME: only PE->LAN UUFlow will enter here!!
			 * need to set net_dev as WAN (such as eth0)
			 */
...
static void cs_ne_init_cfg(void)
{
	 ...
	 */
	writel(0x0829c0d8, NI_TOP_NI_SCH_BP_THLD_ETH);
#else
		...
		if (frag_id == 0) {
			...
			if (page_num == 0) {
				...
				for (i = 0; i < page_num; i++) {
					...
					pkt_datap = page_address(frag->page) +
							frag->page_offset;
					len = frag->size;
					csum = csum_partial(pkt_datap, len, 0);
				}
