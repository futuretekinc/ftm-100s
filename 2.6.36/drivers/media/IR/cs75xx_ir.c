...
static int cs75xx_ir_open(void *priv)
{
	struct cs75xx_ir_device *ir = priv;
...
static void cs75xx_ir_close(void *priv)
{
	struct cs75xx_ir_device *ir = priv;
...
int cs75xx_ir_sw_init(struct cs75xx_ir_device *ir)
{
	...
	if (!input_dev || !ir_props)
		goto err_out_free;

	ir_props->driver_type = RC_DRIVER_SCANCODE;
	ir_props->allowed_protos = IR_TYPE_RC5 | IR_TYPE_NEC;
	ir_props->priv = ir;
	ir_props->open = cs75xx_ir_open;
	ir_props->close = cs75xx_ir_close;
	...
	error = ir_input_register(ir->idev, rc_map_cfg, ir->props, ir->name);
	if (error) {
