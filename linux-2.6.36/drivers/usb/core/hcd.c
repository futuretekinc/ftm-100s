	...
	if (usb_endpoint_xfer_control(&urb->ep->desc)) {
		if (hcd->self.uses_dma) {
			...
			if (cs_acp_enable & CS75XX_ACP_ENABLE_USB) {
				urb->setup_dma = virt_to_phys(urb->setup_packet) | GOLDENGATE_ACP_BASE;
			...
			if (cs_acp_enable & CS75XX_ACP_ENABLE_USB) {
				if (urb->num_sgs) {
					...
					for_each_sg(urb->sg, s, urb->num_sgs, i) {
						s->dma_address = (page_to_dma(hcd->self.controller, sg_page(s)) +
							s->offset) | GOLDENGATE_ACP_BASE;
					}
					urb->transfer_flags |= URB_DMA_SG_COMBINED;
				} else if (urb->sg) {
					struct scatterlist *s = urb->sg;
					urb->transfer_dma = (page_to_dma(hcd->self.controller, sg_page(s)) +
							s->offset) | GOLDENGATE_ACP_BASE;
					urb->transfer_flags |= URB_DMA_MAP_PAGE;
				} else {
					urb->transfer_dma = virt_to_phys(urb->transfer_buffer) | GOLDENGATE_ACP_BASE;
					urb->transfer_flags |= URB_DMA_MAP_SINGLE;
				}
			} else {
#endif
