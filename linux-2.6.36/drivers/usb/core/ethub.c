...
static inline int hub_is_superspeed_port(struct usb_hub *hub, int port1)
{
	...
	if (hdev->descriptor.bDeviceProtocol == 3) {
		if (hdev->parent != NULL)
			ret = 1;
		else if (hcd->rh_usb3_ports & (1 << port1))
			ret = 1;
	}
...
static inline char *portspeed(struct usb_hub *hub, int port1, int portstatus)
{
	if (hub_is_superspeed_port(hub, port1))
		return "5.0 Gb/s";
...
static int hub_port_disable(struct usb_hub *hub, int port1, int set_state)
{
	...
	if (!hub->error && !hub_is_superspeed_port(hub, port1))
...
static bool hub_port_warm_reset_required(struct usb_hub *hub, int port1, u16 portstatus);
 ...
 init2:
	...
	for (port1 = 1; port1 <= hdev->maxchild; ++port1) {
		...
		if ((portchange & USB_PORT_STAT_C_LINK_STATE) &&
				hub_is_superspeed_port(hub, port1)) {
			need_debounce_delay = true;
			clear_port_feature(hub->hdev, port1,
...
static bool hub_port_warm_reset_required(struct usb_hub *hub, int port1, u16 portstatus)
{
	return hub_is_superspeed_port(hub, port1) &&
...
static int port_is_power_on(struct usb_hub *hub, int port1, unsigned portstatus)
{
	int ret = 0;
...
static int port_is_suspended(struct usb_hub *hub, int port1, unsigned portstatus)
{
	int ret = 0;
		...
		int status, unsigned portchange, unsigned portstatus)
{
	...
	if (status || port_is_suspended(hub, port1, portstatus) ||
			!port_is_power_on(hub, port1, portstatus) ||
			!(portstatus & USB_PORT_STAT_CONNECTION)) {
		if (status >= 0)
			status = -ENODEV;
	}
...
int ethub_usb_port_resume(struct usb_device *udev, pm_message_t msg)
{
	...
	status = hub_port_status(hub, port1, &portstatus, &portchange);
	if (status == 0 && !port_is_suspended(hub, port1, portstatus))
		goto SuspendCleared;
					...
					u16 portstatus, u16 portchange)
{
	...
	dev_dbg (hub_dev,
		...
		port1, portstatus, portchange, portspeed (hub, port1, portstatus));
...
static void hub_events(void)
{
	...
	while (1) {
		...
		for (i = 1; i <= hub->descriptor->bNbrPorts; i++) {
			 ...
			 */
			if (hub_port_warm_reset_required(hub, i, portstatus)) {
