			     ...
			     struct mtd_oob_ops *ops)
{
	...
	while (1) {
		...
		if (unlikely(column || writelen < (mtd->writesize - 1))) {
			...
			chip->ecc.read_page(mtd, chip, chip->buffers->databuf, 0,
					    page);
				    ...
				    uint32_t readlen, uint8_t * bufpoi)
{
	...
	chip->ecc.read_page(mtd, chip, chip->buffers->databuf, 0, page);
		...
		if (realpage != chip->pagebuf || oob) {
			...
			if (unlikely(ops->mode == MTD_OPS_RAW))
				ret = chip->ecc.read_page_raw(mtd, chip,
							      bufpoi, 0, page);
			else if (!aligned && (chip->ecc.mode == NAND_ECC_SOFT) && (chip->page_shift > 9) && !oob)
				ret =
				    chip->ecc.read_subpage(mtd, chip, col,
							   bytes, bufpoi);
			else {
				ret =
				    chip->ecc.read_page(mtd, chip, bufpoi, 0,
							page);
			}
			if (ret < 0)
				break;
			...
			if (!aligned) {
				if (!((chip->ecc.mode == NAND_ECC_SOFT) && (chip->page_shift > 9)) && !oob)
			   ...
			   const uint8_t *buf, int oob_required, int page, int cached, int raw)
{
	...
	if (cs752x_nand_verify_buf(mtd, buf, mtd->writesize)){
	printk("%s(mtd, buf, %d)=====>page(%x)--%x\n",__func__, mtd->writesize,page,buf[0]);
...
static void cs752x_nand_read_buf(struct mtd_info *mtd, uint8_t *buf, int len)
{
	...
	if (len <= (mtd->writesize + mtd->oobsize)) {
		...
		this->ecc.read_page(mtd, this, this->buffers->databuf, 0, page);
...
static uint8_t cs752x_nand_read_byte(struct mtd_info *mtd)
{
	...
	cs752x_nand_read_page_raw(mtd, chip, chip->buffers->databuf, 0, page);
