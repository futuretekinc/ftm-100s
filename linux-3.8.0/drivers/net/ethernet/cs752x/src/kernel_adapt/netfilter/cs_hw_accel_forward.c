		...
		cs_core_hmu_value_t *value, u32 status)
{
	...
	} else if ((status == CS_CORE_HMU_RET_STATUS_TIMEOUT_PARTIAL) ||
			(status == CS_CORE_HMU_RET_STATUS_TIMEOUT_ALL) ||
			(status == CS_CORE_HMU_RET_STATUS_DELETED_ALL)) {
		DBG(printk("\t set ct timeout callback to linux original one \n"));
		/* if the hash is deleted by ct timeout, then
		 * ct->timeout.function could be NULL. */
		if (ptr_death_by_timeout != NULL) {
