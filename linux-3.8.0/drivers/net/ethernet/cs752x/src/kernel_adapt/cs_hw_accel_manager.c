...
static char *module_names[32] = {"NULL", "8021Q", "BRIDGE", "IPV4_MULTICAST",
		...
		"QOS_REMARK", "NF_DROP", "WFO_RT3593", "WFO_AR988X"};
		...
		const char *buffer, unsigned long count, void *data)
{
	...
	unsigned char enbl = 0; /* 0: disable, 1: enable */
	...
	/* split each token */
	if (cs752x_str_paser(buf, 2, token_list, &tok_cnt))
		goto IPLIP_INVAL_EXIT;
	...
	case CS_IPLIP_IPC_PE_MIB_EN:
		if (strict_strtoul(token_list[1], 0, &enbl) ||
				enbl > 1)
			goto IPLIP_INVAL_EXIT;
		...
		const char *buffer, unsigned long count, void *data)
{
	...
	/* split each token */
	if (cs752x_str_paser(buf, 3, token_list, &tok_cnt))
		goto IPLIP_INVAL_EXIT;
