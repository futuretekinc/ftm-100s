				 ...
				 unsigned int reg, unsigned int data)
{
	...
	ret = ni_mdio_write(SWITCH_PHY_ADDR, reg, data);
	if (ret < 0) {
		MESSAGE("ni_mdio_write(0, phy=%d, reg=%d, data=0x%04x) = %d\n", 
			SWITCH_PHY_ADDR, reg, data, ret);
	}
}
...
{
	*pdata = ni_mdio_read(SWITCH_PHY_ADDR, reg);
}
