...
static int cs752x_sd_get_cd(struct mmc_host *mmc)
{
	...
	if (0 == present) {
		...
		printk("card is %spresent by detecting CD pin\n", present ? "" : "not ");
	} else {
		...
		printk("card is %spresent by detecting DAT3 pin\n", present ? "" : "not ");
	}
...
static int cs752x_sd_probe(struct platform_device *pdev)
{
...
static int cs752x_sd_remove(struct platform_device *pdev)
{
	struct mmc_host     *mmc  = platform_get_drvdata(pdev);
	struct cs752x_sd_host  *cs752x_host = mmc_priv(mmc);
