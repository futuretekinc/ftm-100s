...
#endif
{
	...
	int error = -ENOSPC;
	uid_t uid = __kuid_val(YCRED(current)->fsuid);
	gid_t gid =
	    (dir->i_mode & S_ISGID) ? __kgid_val(dir->i_gid) : __kgid_val(YCRED(current)->fsgid);
				      ...
				      struct yaffs_obj *obj)
{
	if (inode && obj) {
		...
		yaffs_trace(YAFFS_TRACE_OS,
			...
			inode->i_mode, __kuid_val(inode->i_uid), __kgid_val(inode->i_gid),
